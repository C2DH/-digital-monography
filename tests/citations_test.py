...
from src.scripts.citations import (
    add_citations,
...
class TestCitationsFromFields:
    ...
    def test_extract_citations_properly_decodes_json_data(self):
        ...
        assert (
            self.citations[0]["citationItems"][0]["suffix"] == "for examples"
        )
        assert (
            self.citations[0]["citationItems"][0]["itemData"]["title"]
            == "I disagree"
        )
        assert (
...
class TestCitationsFromFootnotes:
    ...
    def test_extract_citations_properly_decodes_json_data(self):
        ...
        assert (
            self.citations[0]["citationItems"][0]["itemData"]["author"][0][
                "family"
            ]
            == "Schmoe"
        )
        ...
        assert (
            self.citations[1]["citationItems"][0]["itemData"]["type"]
            == "chapter"
        )
        assert (
            self.citations[1]["citationItems"][0]["itemData"]["title"]
            == "I disagree"
        )
        assert (
            self.citations[1]["citationItems"][0]["itemData"]["author"][0][
                "family"
            ]
            == "Ce"
        )
        ...
        assert (
            self.citations[1]["citationItems"][1]["itemData"]["type"] == "book"
        )
        assert (
        ...
        assert (
            self.citations[1]["citationItems"][1]["itemData"]["author"][0][
                "family"
            ]
            == "Ce"
        )
